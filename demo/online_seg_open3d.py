...
class seg3d:
    def __init__(self, RES_X, RES_Y, headless):
        ...
        self.sphere = o3d.visualization.Visualizer()
        self.sphere.create_window('Masked Point Cloud')
    ...
    def visualize_grid_2D(self, distance_matrix):
        # 将二维网格矩阵转换为图像
        img = np.uint8(distance_matrix * 255)  # 将0/1矩阵转换为0/255
    ...
    def visualize_grid_sphere(self,distance_matrix):
        # 将距离矩阵投影到球面上
        points = []
        colors = []
        for v in range(self.VERTICAL_SECTIONS):
            for h in range(self.HORIZONTAL_SECTIONS):
                ...
                if distance > 0:
                    ...
                    azimuth = (h / self.HORIZONTAL_SECTIONS) * 2 * self.PI - self.PI
                    elevation = (v / self.VERTICAL_SECTIONS) * self.PI - self.PI / 2
        ...
        self.point_cloud.points = o3d.utility.Vector3dVector(np.array(points).astype(np.float64))
        self.point_cloud.colors = o3d.utility.Vector3dVector(np.array(colors).astype(np.float64))

        # 可视化
        # o3d.visualization.draw_geometries([point_cloud])
        self.sphere.add_geometry(self.point_cloud)
        self.sphere.update_geometry(self.point_cloud)
        self.sphere.poll_events()
        self.sphere.update_renderer()
...
if __name__ == '__main__':
    ...
    while True:
        start = time.time()
        Seg3D.update()
        distance_matrix=Seg3D.point_cloud_3D()
        Seg3D.visualize_grid_sphere(distance_matrix)
        end = time.time()
