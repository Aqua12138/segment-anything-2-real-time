...
class seg3d:
    def __init__(self, RES_X, RES_Y, headless):
        ...
        self.camera_base = np.array([
            [0, 0, 1, -4.201838970184326],
            [0, -1, 0, 1.8102757930755615],
            [1, 0, 0, 7.685793876647949],
            [0, 0, 0, 1.0]
        ])

        self.pixel_axis = np.array([
            [1, 0, 0, -3.027566432952881],
            [0, 1, 0, 0.76460200548172],
            [0, 0, 1, 7.746606826782227],
            [0, 0, 0, 1.0]
        ])
    ...
    def point_cloud_3D(self):
        # 初始化距离矩阵，填充为极大值
        grid = np.full((self.VERTICAL_SECTIONS, self.HORIZONTAL_SECTIONS), np.inf)
        ...
        for point in self.transform_point_cloud().T:
            ...
            # 将方位角和俯仰角转换为球形网格索引
            horizontal_index = int((azimuth + self.PI / 2) / (2 * self.PI) * self.HORIZONTAL_SECTIONS) % self.HORIZONTAL_SECTIONS
            vertical_index = int((elevation + self.PI / 2) / self.PI * self.VERTICAL_SECTIONS) % self.VERTICAL_SECTIONS

            # 保留每个网格中距原点最近的点
            if distance < grid[vertical_index, horizontal_index]:
                grid[vertical_index, horizontal_index] = distance
        ...
        for v in range(self.VERTICAL_SECTIONS):
            for h in range(self.HORIZONTAL_SECTIONS):
                distance = grid[v, h]
                # 归一化处理并且距离大于1时设为0
                grid[v, h] = max(0.0, 1 - distance / 2) if distance != np.inf else 0.0

        return grid

    def visualize_grid_3D(self, grid):
        # 将二维网格矩阵转换为图像
        img = np.uint8(grid * 255)
        img = cv2.applyColorMap(img, cv2.COLORMAP_VIRIDIS)# 将0/1矩阵转换为0/255
        img = cv2.resize(img, (1000, 500), interpolation=cv2.INTER_NEAREST)  # 放大图像以便可视化
    ...
    def render(self):
        ...
        # np.save("/home/zhx/PycharmProjects/draw/point_cloud/bottle.npy", self.filtered_points)
...
if __name__ == '__main__':
    ...
    while True:
        start = time.time()
        Seg3D.update()
        grid=Seg3D.point_cloud_3D()
        Seg3D.visualize_grid_3D(grid)
        end = time.time()
